...
def display_log():
    ...
    def export_project():
        ...
        for item in tree.get_children():
            temp_values = [tree.item(item)["text"]]
            for i in (tree.item(item)["values"]):
                temp_values.append(i)
            values_to_export.append(temp_values)

        for i in values_to_export:
            split_time = re.split('hr |min', i[1])
            i[1] = int((int(split_time[0]) * 60) + int(split_time[1]))

        f_loc = asksaveasfilename(filetypes = [('.xlsx', '*.xlsx')], defaultextension = '*.xlsx')

        while f_loc[-5:] != '.xlsx' and len(f_loc) > 0:
            messagebox.showwarning("Invalid Name", "Please enter a valid name.\nMust have '.xlsx' filetype.")
            f_loc = asksaveasfilename(filetypes = [('.xlsx', '*.xlsx')], defaultextension = '*.xlsx')

        if len(f_loc) > 0:
            workbook = xlsxwriter.Workbook(f_loc)
            bold = workbook.add_format({'bold': True})

            worksheet = workbook.add_worksheet('Project - ' + filter_value_global)

            worksheet.write(0, 0, "Project", bold)
            worksheet.write(0, 1, "Time (mins)", bold)
            worksheet.write(0, 2, "Comment", bold)
            worksheet.write(0, 3, "Date", bold)
            worksheet.write(0, 4, "Record ID", bold)

            row = 1
            for rec in values_to_export:
                col = 0
                for val in rec:
            ...
            try:
                ...
                messagebox.showwarning("File already open", "The file you selected is already open in another program.\nPlease close it and try again.")
    ...
    export_btn = Button(filter_window, text = "Export", takefocus = False, width = 17, style = 'my.TButton',
                      command = export_project) #TODO Add export command
